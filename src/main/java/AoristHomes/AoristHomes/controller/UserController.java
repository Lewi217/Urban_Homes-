...
public class UserController {
    ...
    public ResponseEntity<ApiResponse> getHoldings(@PathVariable String userId) {
        try {
            List<HoldingDTO> holdings = userService.getHoldingsByUserId(userId);
            return ResponseEntity.ok(new ApiResponse(REQUEST_SUCCESS_MESSAGE, holdings));
        } catch (Exception e) {
            return ResponseEntity
                    .badRequest()
                    .body(new ApiResponse(REQUEST_ERROR_MESSAGE, e.getMessage()));
